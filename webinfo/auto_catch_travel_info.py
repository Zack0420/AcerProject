...
def get_urlList(object):
    ...
    while controler:
        ...
        try:
            ...
            WebDriverWait(object, 10, 0.5).until(EC.visibility_of_element_located((By.XPATH, '//*[@id="dataT-Main"]')))
            WebDriverWait(object, 10, 0.5).until(EC.visibility_of_element_located((By.XPATH, '//*[@id="PageSelector"]/span')))
        ...
        currentPage = object.find_element(By.XPATH, '//*[@id="PageSelector"]/span').text
        ...
        #點擊下一頁 
        ActionChains(object).move_to_element(object.find_element(By.XPATH, '//*[@id="tq-dataTourjs"]/div[4]/div[1]/btn[2]')).click().perform()
...
def get_one_info(url):
    ...
    try:
        ...
        info_dict['title'] = title.split('\r\n')[0]
        
        info_dict["travel_company"] = "可樂旅遊"
...
def get_all_plans_info(urlList, regionList):
    ...
    for i in urlList:
        ...
        travel_company_List.append(info['travel_company'])
    ...
    all_info = pd.DataFrame({'travel_company': travel_company_List,
                             'area': regionList,
                             'title':title_List,
                             'price':price_List,
                             'date':date_List,
...
def get_search_result_urlList():
    ...
    for bound in bound_type:
        ...
        if bound == "in":
            ...
            page_urlList = list(get_urlList(browser))
            temp_region = ["台灣" for i in range(len(page_urlList))]
            urlList += page_urlList
            regionList += temp_region
            browser.back()
            #等待搜尋條載入
            try:
                WebDriverWait(browser, 10, 0.5).until(EC.presence_of_element_located((By.XPATH, '//*[@id="searchbar"]/div/div[3]')))
            ...
            #抓取地區
            region_options = [region.text for region in browser.find_elements(By.XPATH, '//*[@id="searchbar"]/div/div[2]/div[1]/div[3]/div/header/ul/li')[:-1]]
            new_region_name = ["東北亞", "東南亞", "大陸港澳", "歐洲", "美洲", "大洋洲", "其他"]
            ...
            for index, region in enumerate(region_options):
                ...
                for b_index, block in enumerate(browser.find_elements(By.XPATH, '//*[@id="searchbar"]/div/div[2]/div[1]/div[3]/div/main/div')):
                    for c_index, country in enumerate(browser.find_elements(By.XPATH, '//*[@id="searchbar"]/div/div[2]/div[1]/div[3]/div/main/div[{}]/div'.format(b_index + 1))):
                        ...
                        page_urlList = list(get_urlList(browser))
                        temp_region = [new_region_name[index] for i in range(len(page_urlList))]
                        urlList += page_urlList
                        regionList += temp_region
                        browser.back()
    ...
    browser.close()
    browser.quit()                           
    return urlList, regionList

def main():
    
    urlList, regionList = get_search_result_urlList()
    info = get_all_plans_info(urlList, regionList)
