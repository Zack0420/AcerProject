...
try:
    ...
    for time in range(title_count): #例53項
            ...
            print(UrlList[time])
            WebDriverWait(browser, 10, 0.1).until(EC.presence_of_all_elements_located(date_locator))  #以列表顯示
            ...
            try:
                ...
                for ati in all_trip_info:     
                    ...
                    for fd,tn,tss,sa,ts,tse,p in zip(fromDate, trip_num, trip_state_show, sellable, totalsell, trip_state, price):
                        ...
                        #旅館資訊 
                        try:       
                            hotel_info = browser.find_element(By.XPATH,'//*[@id="schedules"]/div[2]').find_elements(By.TAG_NAME,"div") 
                            count=1
                            for hotelinfo in hotel_info:
                                hotels = hotelinfo.find_elements(By.XPATH, "div/div[3]/div[2]/div/span")
                                for hotel in hotels:
                        ...
                        try:
                            ...
                            for ftm in full_trip_message:
                                ...
                                for ft in ftmf:
                                    print(ft.text)
                                    full_trip_message_List.append(ft.text)
                        except:
                            full_trip_message_List.append("無行程資料")
                              
            except:
                    print("無出發日=====all_trip_info2")
                    inside_titleList.append(titleList[time])
...
data = pd.DataFrame({"旅遊公司名稱":typeList,
                     ...
                     "價格":priceList,
                     "住宿點":hotelList})
data.to_csv("旅遊搜尋_大阪.csv",encoding="utf-8")
