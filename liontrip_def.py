...
def fetch_page_data(browser,total):
    """ 抓取當前頁面的數據 """
    # global title_count, page, checkDateFlag
    ...
    while True:
        ...
        for title, url, price in zip(titles, titleUrls, prices):
            ...
            title_priceList.append(price.text)
...
def fetch_inside_information(browser):
    ...
    for index, url in enumerate(UrlList):
        try:
            ...
            print(url)
            WebDriverWait(browser, 20).until(EC.presence_of_element_located(inside_locator))
            ...
            main_bar = WebDriverWait(browser, 10).until(EC.presence_of_element_located((By.XPATH, '//*[@id="root"]/div/div[4]/div/div[1]/div[3]/div/div[2]/div/div[2]/div[1]/section[1]')))
            ...
            for trip_info in all_trip_info:
                ...
                fromDate = trip_info.find_elements(By.XPATH,'section[2]/div/div[1]') #出發日期o
                trip_num = trip_info.find_elements(By.XPATH,'section[2]/div/div[2]') #行程編號o
                trip_state_show = trip_info.find_elements(By.XPATH,'section[2]/div/div[3]') #若以成行show成行，若無為空o
                sellable = trip_info.find_elements(By.XPATH,'section[2]/div/div[7]') #剩餘可賣o
                totalsell = trip_info.find_elements(By.XPATH,'section[2]/div/div[8]') #總席次o
                trip_state = trip_info.find_elements(By.XPATH,'section[2]/div/div[5]') #報名狀態o
                price = trip_info.find_elements(By.XPATH,'section[2]/div/div[9]') #價格o
                ...
                for fd,tn,tss,sa,ts,tse,p in zip(fromDate, trip_num, trip_state_show, sellable, totalsell, trip_state, price):
                    #標題o
                    print(str(index+1)+".", titleList[index]) 
                    inside_titleList.append(titleList[index])
                    
                    #連結o
                    print(UrlList[index]) 
                    inside_urlList.append(UrlList[index])
                    ...
                    #出發資訊o
                    try:
                        flying_and_from_info = browser.find_element(By.XPATH, '//*[@id="root"]/div/div[4]/div/div[2]').find_element(By.CLASS_NAME, "contents-list--1wwon") #出發地點資訊
                        print(flying_and_from_info.text)
                        transportation.append(flying_and_from_info.text) #出發資訊
                        ...
                        for fai in flying_and_from_info:
                            print(fai.text)
                        transportation.append(fai.text) #出發資訊
                    except:
...
def main():
    ...
    global area
    # browser_url = ["https://travel.liontravel.com/search?DepartureID=&ArriveID=--9,&GoDateStart="+departure_time1+"&GoDateEnd="+departure_time2+"&Keywords=&TravelType=1&IsSold=true&Platform=APP",
    #                 "https://travel.liontravel.com/search?DepartureID=&ArriveID=--6,&GoDateStart="+departure_time1+"&GoDateEnd="+departure_time2+"&Keywords=&TravelType=1&IsSold=true&Platform=APP",
    #                 "https://travel.liontravel.com/search?DepartureID=&ArriveID=--5,&GoDateStart="+departure_time1+"&GoDateEnd="+departure_time2+"&Keywords=&TravelType=1&IsSold=true&Platform=APP",
    #                 "https://travel.liontravel.com/search?DepartureID=&ArriveID=--7,&GoDateStart="+departure_time1+"&GoDateEnd="+departure_time2+"&Keywords=&TravelType=1&IsSold=true&Platform=APP",
    #                 "https://travel.liontravel.com/search?DepartureID=&ArriveID=--3,&GoDateStart="+departure_time1+"&GoDateEnd="+departure_time2+"&Keywords=&TravelType=1&IsSold=true&Platform=APP",
    #                 "https://travel.liontravel.com/search?DepartureID=&ArriveID=--1,&GoDateStart="+departure_time1+"&GoDateEnd="+departure_time2+"&Keywords=&TravelType=1&IsSold=true&Platform=APP",
    #                 "https://travel.liontravel.com/search?DepartureID=&ArriveID=--4,&GoDateStart="+departure_time1+"&GoDateEnd="+departure_time2+"&Keywords=&TravelType=1&IsSold=true&Platform=APP",
    #                 "https://travel.liontravel.com/search?DepartureID=&ArriveID=--2,&GoDateStart="+departure_time1+"&GoDateEnd="+departure_time2+"&Keywords=&TravelType=1&IsSold=true&Platform=APP"]
    
    browser_url = [("https://travel.liontravel.com/search?DepartureID=&ArriveID=--9,&GoDateStart="+departure_time1+"&GoDateEnd="+departure_time2+"&Keywords=&TravelType=1&IsSold=true&Platform=APP", "台灣"),
                   ...
                   ("https://travel.liontravel.com/search?DepartureID=&ArriveID=--4,&GoDateStart="+departure_time1+"&GoDateEnd="+departure_time2+"&Keywords=&TravelType=1&IsSold=true&Platform=APP", "其他"),
    ...
    try:
        for url, area in browser_url:
                ...
                data = pd.DataFrame({
                    ...
                    "remaining_quota": tripGroupState2,#成團狀況(可售)
                    "tour_schedule ":tour_schedual_List,
                    "url": inside_urlList,
                    })
            
                data.to_csv("liontrip_0607def_1.csv", encoding="utf-8", index=False)
