    ...
    for (let i = 0; i <= days_of_month[month] + first_day.getDay() - 1; i++) {
        ...
        if (i >= first_day.getDay()) {
            ...
            if (i - first_day.getDay() + 1 === currDate.getDate() && year === currDate.getFullYear() && month === currDate.getMonth()) {
                ...
                // 顯示當天行程
                fetchAndDisplaySchedule(currDate);
            }
            day.addEventListener('click', () => {
                // 移除之前選中的日期的 class
                let selectedDay = document.querySelector('.calender-day-hover.onclick');
                if (selectedDay) selectedDay.classList.remove('onclick');

                // 為點擊的日期添加 class
                day.classList.add('onclick');

                // 抓取選中的日期並回傳後端
                let selectedDate = new Date(year, month, i - first_day.getDay() + 1);
...
function fetchAndDisplaySchedule(date) {
    ...
    fetch(`/schedule?date=${formattedDate}`)
...
function renderDailySchedule(schedule) {
    ...
    schedule.forEach((item, index) => {
        ...
        `;
        $dailySchedule.appendChild($accordionItem);
    });
}
