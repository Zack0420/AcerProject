...
def home(request):
    ...
    if "username" not in request.COOKIES.keys():
        ...
        return render(request, "member/index.html", {"username": request.COOKIES["username"]})
...
def signupForm(request):
    ...
    if "usermsg" not in request.COOKIES.keys():
        ...
        response = render(request, "member/signup.html", {"usermsg": usermsg, "cfpmsg":cfpmsg, "emailmsg":emailmsg})
        return response
    else:
...
def loginForm(request):
    if "usermsg" not in request.COOKIES.keys():
        usermsg = False
        pwdmsg = False
        response = render(request, "member/login.html", {"usermsg": usermsg, "pwdmsg":pwdmsg})
        return response
...
def profile(request):
    ...
    if "username" in request.COOKIES.keys():
        ...
        return render(request, "member/profileinfo.html", {"username":username, "email":email})
...
def verifyEmail(request):
    ...
    if "email" in request.COOKIES and "username" in request.COOKIES:
        ...
        html_content = '<a href = "https://acerproject-2722811458ca.herokuapp.com/member/change_verifiction_status/%s">Pleask click here to verify your email!</p>'%username
...
def resetInfoForm(request):
    ...
    if "usermsg" not in request.COOKIES.keys():
        ...
        response = render(request, "member/resetInfo.html", {"usermsg": usermsg, "emailmsg":emailmsg})
        return response
    else:
...
def sendResetEmail(request):
    ...
    if "email" in request.COOKIES and "username" in request.COOKIES:
        ...
        html_content = '<a href = "https://acerproject-2722811458ca.herokuapp.com/member/change_verifiction_status/%s">Pleask click here to reset your password!</p>'%username
        msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
        msg.attach_alternative(html_content, "text/html")
        msg.send()
...
def resetForm(request, username):
    ...
    if "cfpmsg" not in request.COOKIES.keys():
        ...
        response = render(request, "member/resetForm.html", {"cfpmsg":cfpmsg, "username":username})
        return response
    else:
