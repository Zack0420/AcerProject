...
def group_travel_search_in():
    ...
    for departure_index, departure_option in enumerate(departure_options, start = 1):
        ...
        for url in newurls:
            ...
            #抓取的內容
            data["travel_company"] = "五福旅遊"
            data["area"] = "台灣"
            data["title"] = soup.select_one("div.prod-name").text.strip().replace("【", " ").replace("】", " ")
            try:
                price = soup.select_one("div.price-bar")
                for div in price.find_all("div")[1:2]:
                    data["price"] = int(div.div.text.replace(',',''))
            except:
                data["price"] = ''
            try:
                ...
                # 從soup中提取到的日期字串
                date_str = soup.find("div", string="出發日期").find_next_sibling("div").text.strip()
                #格式轉換
                month_day = date_str.split('月')
                month = month_day[0].zfill(2)  # 確保月份是兩位數
                day = month_day[1].replace('日', '').zfill(2)  # 確保日期是兩位數
            ...
            try:
                ...
                data["departure_city"] = departure_option
            try:
                data["duration"] = int(soup.find("div", string="旅遊天數").find_next_sibling("div").text.strip().split('天')[0])
            except:
                data["duration"] = ''
            # try:
            ...
            if schedule_section:
                ...
                for day_schedule in schedule_section.find_all("div", class_="schedule-box"):
                    ...
                    schedule = [part.strip().replace("【", " ").replace("】", " ") for part in schedule_text.split("＞") if part.strip()]
                ...
                if continents_index in [8,12]:
                    ...
                    for  region_index, region_option in enumerate(region_options, start = 1):
                        ...
                        for url in newurls:
                            ...
                            data["travel_company"] = "五福旅遊"
                            try:
                                if continents_option == '紐澳大洋':
                                    data["area"] = '大洋洲 '
                                elif continents_option == '中東非洲' and '太平洋小島' and '中南亞' and "高球假期":
                                    data["area"] = '其他 '
                                else:
                                    data["area"] = continents_option
                            except:
                                data["area"] = ''
                            try:
                                data["title"] = soup.select_one("div.prod-name").text.strip().replace("【", " ").replace("】", " ")
                            ...
                            try:
                                ...
                                if schedule_section:
                                    ...
                                    for day_schedule in schedule_section.find_all("div", class_="schedule-box"):
                                        ...
                                        # 使用相對路徑選取包含行程資訊的文本內容
                                        schedule_xpath = day_schedule.find("div", class_="head")
                                        schedule_xpath = day_schedule.find("div", class_="wrap-day")
                                        if schedule_xpath:
                                            schedule_head = schedule_xpath.find("h3", class_="head")
                                            if schedule_head:
                                                schedule_text = schedule_head.get_text(separator="\n", strip=True)
                                                # 清理文本,移除多餘的空白字符和轉義字符
                                                schedule_text = ' '.join(schedule_text.split()).replace('\u3000', '')
                                                schedule = [part.strip().replace("【", " ").replace("】", " ") for part in schedule_text.split("＞") if part.strip()]
                                            else:
        ...
        #--------------------------------------------------------------------
            ...
            for url in newurls:
                ...
                try:
                    ...
                    data["remaining_quota"] = "此為自由行方案"

                # 取得行程資訊
                try:
                    hotel_section = soup.find("section", {"id": "hotel"})
                    hotel_name = hotel_section.find('div', class_='item-title').text
                    hotel_url = hotel_section.find('div', class_='item-title').find('a').get('href')
                    hotel_staydates = '住'+hotel_section.find('div', class_='staydates').text+'晚'
                    try:
                        hotel_price = hotel_section.find('div', class_='price').text+'元起'
                    except:
                        hotel_price = '額滿'
                    # print(hotel_name)
                    # print(hotel_url)
                    # print(hotel_staydates)
                    # print(hotel_price)
        
                    data["tour_schedule"] = {"schedule": [],"possible_hotel": [hotel_name, hotel_staydates, hotel_price, hotel_url]}
                except:
                    data["tour_schedule"] = ''
...
def all_search():
    ...
    #     if result['date'][i][-2:] == "00":
    #         result['date'][i] = ""
    
    result = result.apply(lambda x: x.apply(replace_brackets) if x.dtype == object else x)
    
    # file_name = "lifetour_search"+current_datetime.strftime('%m%d')+".csv"
    # if result.empty:
    #     print(f"{file_name}沒有資料")
    # else:
    #     result.to_csv(file_name, index=True, encoding='UTF-8')
    #     print(f'文件已保存為: {file_name}')
